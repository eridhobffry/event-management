...
const constructEvent = vi.fn((_raw: string, _sig: string, _secret: string) => ({
  ...
  data: { object: { id: "pi_2", status: "succeeded", currency: "eur", amount: 2000, metadata: { order_id: "order_2" } } },
}));
class FakeStripe { webhooks = { constructEvent }; }
vi.mock("stripe", () => ({ default: FakeStripe }));

let orderStatus: "pending" | "paid" = "pending";
const orderRow = { id: "order_2", status: orderStatus, eventId: "evt_app_1", email: "buyer2@example.com" };
const eventRow = { name: "Deep Event", date: new Date(), location: "Paris" };
