...
export default function CheckInClient() {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!t) {
      setResult({
        ok: false,
        error: "No token found. Paste token or QR link with ?token=...",
      });
      return;
    }
    setLoading(true);
  ...
  const startCamera = useCallback(async () => {
    ...
    try {
      ...
      // Ensure the <video> element exists before using it: render it and wait a frame
      setScanning(true);
      ...
      const BarcodeDetectorCtorRef = (
        window as unknown as { BarcodeDetector?: BarcodeDetectorCtor }
      ).BarcodeDetector;
      if (!BarcodeDetectorCtorRef)
  ...
  return (
    <div className="max-w-xl mx-auto p-4">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Paste a QR link or token. We will toggle check-in for the matching
        ticket.
      </p>
        ...
        <div className="min-h-[56px]">
          {result && result.ok && (
            <div className="space-y-2">
              ...
              <div className="text-xs text-muted-foreground">
                {result.checkedInAt
                  ? `Time: ${new Date(result.checkedInAt).toLocaleString()}`
                  : "Not checked in"}
              </div>
            </div>
          )}
      ...
      <div className="text-sm text-muted-foreground mt-4">
        Tip: Camera scanning uses the native BarcodeDetector. If unsupported,
        paste the QR link or token.
      </div>
    </div>
  );
}
